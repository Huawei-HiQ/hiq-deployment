...
Source0:        https://cmake.org/files/v%{major_version}.%{minor_version}/cmake-%{version}%{?versuf}.tar.gz
Source1:        %{name}-init.el
Source2:        cmake-macros.%{name}
...
%endif
# RPM macros
install -p -m0644 -D %{SOURCE2} %{buildroot}%{rpm_macros_dir}/cmake-macros.%{name}
sed -i -e "s|@@CMAKE_VERSION@@|%{version}|" -e "s|@@CMAKE_MAJOR_VERSION@@|%{major_version}|" %{buildroot}%{rpm_macros_dir}/cmake-macros.%{name}
touch -r %{SOURCE2} %{buildroot}%{rpm_macros_dir}/cmake-macros.%{name}
...
/usr/share/doc/cmake/*/LICENSE
/usr/share/doc/cmake/*/COPYING
/usr/share/doc/cmake/*/Copyright.txt
%endif
...
%files rpm-macros
%{rpm_macros_dir}/cmake-macros.%{name}
%if %{with rpm} && 0%{?_rpmconfigdir:1}
