...
export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$PWD/blosc/
...
%install
%if 0%{?mdkver}
DESTDIR="%{buildroot}" %__cmake --install "${CMAKE_BUILD_DIR:-build}"
%else
%cmake_install
%endif
...
%if 0%{?mdkver}
install -p ${CMAKE_BUILD_DIR:-build}/bench/bench -D %{buildroot}/%{_bindir}/%{name}-bench
%else
