...
function pkg_download()
{
	...
	wget https://github.com/numpy/numpy/releases/download/v1.18.4/numpy-1.18.4.tar.gz 1>&2
...
10
EOF
	sed -i -e 's/debhelper-compat (= 11)/debhelper/' \
...
function pkg_update_changelog()
{
    ...
    ver_prefix=$(dpkg-parsechangelog -c 1 -l $pkg_dir/debian/changelog -S Version \
		     | cut -d':' -f1)
    version=$(gen_version $pkg_dir $ubuntu_num)

    if [[ "$ver_prefix" != "$version" ]]; then
	version="${ver_prefix}:$version"
    fi
