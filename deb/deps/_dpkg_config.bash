...
function pkg_download()
{
    ...
    orig_tar_xz=$root/${pkg_name}_${pkg_ver}ubuntu3.tar.xz
    if [ ! -f $orig_tar_xz ]; then
        wget http://archive.ubuntu.com/ubuntu/pool/main/d/${pkg_name}/${pkg_name}_${pkg_ver}ubuntu3.tar.xz -P $root 1>&2
    fi
    tar xvf $root/${pkg_name}_${pkg_ver}ubuntu3.tar.xz -C $root 1>&2
    mv -v $root/${pkg_name}-${pkg_ver}ubuntu2 $root/${pkg_name}-${pkg_ver} 1>&2
...
EOF
        ...
        sed -i -e '/# Workaround an error (not finding DEB files for XXX-dbgsym packages)/,+1d' \
            -e '/# Workaround an error (not finding DEB files for XXX-dbgsym packages)/,+1d' \
